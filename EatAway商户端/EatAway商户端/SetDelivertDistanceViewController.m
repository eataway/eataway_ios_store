//
//  SetDelivertDistanceViewController.m
//  EatAway商户端
//
...
@interface SetDelivertDistanceViewController ()<UITextFieldDelegate>
{
    UITextField *TFContent;
}
@end

@implementation SetDelivertDistanceViewController
...
- (void)viewDidLoad
{
    ...
    labelTitle.text = ZEString(@"请输入修改信息", @"Please enter the delivery distance");
...
-(void)btnDoneClick
{
    ...
    {
        [TotalFunctionView alertContent:ZEString(@"请输入配送距离", @"Delivery distance can't be empty") onViewController:self];
    }
    else
    {
        [self.delegate SetDeliveryDistance:TFContent.text];
        [self.view removeFromSuperview];
    }
}
