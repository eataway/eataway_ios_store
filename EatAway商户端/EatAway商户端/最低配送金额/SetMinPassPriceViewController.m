//
//  SetMinPassPriceViewController.m
//  EatAway商户端
//
...
@interface SetMinPassPriceViewController ()<UITextFieldDelegate>
{
    UITextField *TFContent;
}
@end

@implementation SetMinPassPriceViewController
...
- (void)viewDidLoad
{
    ...
    labelTitle.text = ZEString(@"请输入修改信息", @"Please enter a number");
    ...
    UIView *viewTFBG = [[UIView alloc]initWithFrame:CGRectMake(120, 50 + 22, 100, 36)];
    ...
    UILabel *labelKM = [[UILabel alloc]initWithFrame:CGRectMake(80, 50 + 22, 35, 36)];
    labelKM.text = @"$";
    labelKM.font = [UIFont systemFontOfSize:17];
    labelKM.textColor = [UIColor blackColor];
    [viewWihteBG addSubview:labelKM];
    
    TFContent = [[UITextField alloc]initWithFrame:CGRectMake(120, 50 + 22, 100, 36)];
...
-(void)btnDoneClick
{
    ...
    {
        [self.delegate SetMinPassPrice:TFContent.text];
        [self.view removeFromSuperview];
    }
}
-(void)btnDeleteClick
{
   // [self.delegate deleteMinPrice];
    [self.view removeFromSuperview];
}
