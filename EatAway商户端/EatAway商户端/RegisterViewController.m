...
-(void)viewInit
{
    ...
    [btnForRead addTarget:self action:@selector(jumpToService) forControlEvents:UIControlEventTouchUpInside];
...
-(void)btnSendCheckNumClick:(UIButton *)btn
{
    ...
    {
        [TotalFunctionView alertContent:ZEString(@"您输入的手机号码有误", @"wrong phone number") onViewController:self];
    }
    else
    {
...
-(void)btnRegisterClick
{
    ...
    else
    {
        ...
        [SMSSDK commitVerificationCode:checkNumView.textField.text phoneNumber:strCheckedPhoneNum zone:@"61" result:^(NSError *error) {
            ...
            {
                progress = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
                [[ObjectForRequest shareObject] requestWithURL:@"index.php/server/user/register" parameter:@{@"phone":viewUsername.textField.text,@"password":[blockSelf md5:[NSString stringWithFormat:@"%@EatAway",viewPassword.textField.text]]} resultBlock:^(NSDictionary *resultDic) {
                    [progress hide:YES];
                    if(resultDic == nil)
                    {
